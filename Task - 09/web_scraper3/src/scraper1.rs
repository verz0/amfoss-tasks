...
fn crypto(url: &str) {
    ...
    wtr.write_record(&["Rank", "name", "price", "change", "volume"]).unwrap();
    ...
    for element in document.select(&row_selector) {
        ...
        let name_element=element.select(&name).next().unwrap();
        ...
        wtr.write_record([&RANK, &NAME, &PRICE, &CHANGE, &VOLUME]).expect("wtr.write");
